...
public class LustreKaleidoscope02 extends AnimationApplet {
	...
	public void setupInternal() {
		...
		// logo = loadImage("data/Lustre_Logo.jpg");
		...
		for (int pn = 0; pn < kNbrParticles; ++pn) {
			float x = cx + sin(random(3.14152f * 2f)) * width / 2;
			float y = cy + cos(random(3.14152f * 2f)) * width / 2;
			trinkets[pn] = new Particle(x, y, kMinRadius + random(kMaxRadius - kMinRadius), random(256), 192,
					192 + random(64));
		}
	}

	float kGravity = 15;
	float kSpinRate = 0.066f;
		...
		for (int i = 0; i < kNbrRenderIterations; ++i) {
			...
			for (int pn1 = 0; pn1 < kNbrParticles - 1; ++pn1) {
				...
				for (int pn2 = pn1 + 1; pn2 < kNbrParticles; ++pn2) {
					...
					float rLen = (p1.r + p2.r) * .51f;

					if (dLen < rLen) {
						float diff = (dLen - rLen) / dLen;
						// if (sn == 0) println("ay = " + s.a.y + ", by = " +
						// s.b.y + ", dlen = " + dLen + ", rLen = " +
						// s.restLength + ", diff = " + diff);
						diff *= 0.5;
						dx *= diff;
						dy *= diff;
						p1.x += dx;
	...
	public void draw() {
		...
		imageMode(CENTER);
		translate(getWidth()*3f/4f + 25f, getHeight()/4f + 35f);
		rotation += speed;
		if (rotation > TWO_PI) {
			rotation -= TWO_PI;
		}
