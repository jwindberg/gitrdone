...
public class LustreKaleidoscope03 extends AnimationApplet {
	Random RANDOM = new Random();
	int delay = 1000 * 30;
	long resetTime = System.currentTimeMillis() + delay;

	private static final long serialVersionUID = 1L;

	/*
	 * SinusPlasma An oldschool demo effect, reborn Left click to change
	 * gradient colors, right click to change plasma curve
	 * 
	 * Joerg Reuter, jore[at]stachelig[dot]de Creative Commons Attribution 3.0
	 * license http://stachelig.de
	 */

	// global parameters ... feel free to tweak
	// effect is largely tied to screen size, because of the simplistic plasma
	// function
	// drop me an e-mail for an improved version that renders to a OpenGL
	// texture and scales it up
	...
	public void draw() {
		...
		int swingT = swing(t); // swingT/-Y/-YT variables are used for a little
								// tuning ...
		...
		for (int y = 0; y < height; y++) {
			...
			for (int x = 0; x < width; x++) {
				...
				pixels[i++] = gradient(swing(swing(x + swingT) + swingYT) + swing(swing(x + t) + swingY));
			}
		}
		updatePixels();
		translate(3f * getWidth() / 4f, getHeight() / 2f);
		rotate(rotation);
		rotation += speed;
		if (rotation > TWO_PI) {
			rotation -= TWO_PI;
	...
	void makeSwingCurve(int arrlen, int maxval) {
		...
		for (int i = 0; i < arrlen; i++) {
			float ni = i * TWO_PI / arrlen; // ni goes [0..TWO_PI] -> one
											// complete cos wave
			swingCurve[i] = (int) (cos(ni * factor1) * cos(ni * factor2) * cos(ni * factor3) * halfmax + halfmax);
		}
	}
	...
	private void makeGradient(int arrlen) {
		...
		if (colorGrad == null) {
			...
			System.out.println("Gradient factors(" + rf + "," + gf + "," + bf + "), displacement(" + rd + "," + gd + ","
					+ bd + ")");
		}
