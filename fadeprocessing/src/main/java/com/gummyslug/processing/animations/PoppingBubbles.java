...
public class PoppingBubbles extends AnimationApplet {

	private static final long serialVersionUID = 1L;

	/*
	 * Allie Cashel - May 11, 2010 Final CSMC 117 Project Based on the Bored.com
	 * Game: Boomshine Thank you to Keith O'Hara for help with the code. My
	 * current high score is 109... Can you beat it?
	 */
	...
	class Bubble {
		...
		// Tells the Bubbles to shrink once they reach the biggest possible
		// diameter
		void die() {
			if (diameter == 75) {
				growing = false;
		...
		if (key == 's') {
			...
			for (int i = 0; i < my_bubbles.length; i++) {
				for (int j = 0; j < my_bubbles.length; j++) {
					{
						float distance = sqrt(
								sq(my_bubbles[i].x - my_bubbles[j].x) + sq(my_bubbles[i].y - my_bubbles[j].y));
				...
				if (17 * 1000 - millis() <= 1000) {
					for (int a = 0; a < my_bubbles.length; a++) {
						for (int b = 0; b < my_bubbles.length; b++) {
							// Stop motion of bubbles so they stop expanding and
							// the score doesn't keep growing
							float x = 0;
							float y = 0;
							float dx = 0;
							float dy = 0;
