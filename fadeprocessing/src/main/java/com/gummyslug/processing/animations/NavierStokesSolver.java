...
public class NavierStokesSolver {
	...
	double[] getInverseWarpPosition(double x, double y, double scale) {
		...
		result[0] = (cellU > 0.5) ? lerp(u[INDEX(cellX, cellY)], u[INDEX(cellX + 1, cellY)], cellU - 0.5) : lerp(
				u[INDEX(cellX - 1, cellY)], u[INDEX(cellX, cellY)], 0.5 - cellU);
		result[1] = (cellV > 0.5) ? lerp(v[INDEX(cellX, cellY)], v[INDEX(cellX, cellY + 1)], cellU) : lerp(
				v[INDEX(cellX, cellY)], v[INDEX(cellX, cellY - 1)], 0.5 - cellV);
	...
	void advect(int b, double[] d, double[] d0, double[] u, double[] v, double dt) {
		...
		for (i = 1; i <= N; i++) {
			for (j = 1; j <= N; j++) {
				...
				t1 = y - j0;
				t0 = 1 - t1;
				d[INDEX(i, j)] = s0 * (t0 * d0[INDEX(i0, j0)] + t1 * d0[INDEX(i0, j1)]) + s1
	...
	void project(double[] u, double[] v, double[] p, double[] div) {
		...
		for (i = 1; i <= N; i++) {
			for (j = 1; j <= N; j++) {
				div[INDEX(i, j)] = -0.5 * h
						* (u[INDEX(i + 1, j)] - u[INDEX(i - 1, j)] + v[INDEX(i, j + 1)] - v[INDEX(i, j - 1)]);
				p[INDEX(i, j)] = 0;
			}
		}
		set_bnd(0, div);
		...
		for (k = 0; k < 20; k++) {
			for (i = 1; i <= N; i++) {
				for (j = 1; j <= N; j++) {
					p[INDEX(i, j)] = (div[INDEX(i, j)] + p[INDEX(i - 1, j)] + p[INDEX(i + 1, j)] + p[INDEX(i, j - 1)] + p[INDEX(
							i, j + 1)]) / 4;
				}
			}
			set_bnd(0, p);
