...
public class SmokeDynamics extends AnimationApplet {
	...
	private void fluidCanvasStep() {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				double warpedPosition[] = fluidSolver.getInverseWarpPosition(u, v, scale);
	...
	public int getSubPixel(double warpX, double warpY) {
		...
		try {
			return lerpColor(lerpColor(pixels[indexTopLeft], pixels[indexTopRight], (float) u),
					lerpColor(pixels[indexBottomLeft], pixels[indexBottomRight], (float) u), (float) v);
		} catch (Exception e) {
			System.out.println("error caught trying to get color for pixel position " + x + ", " + y);
			return bordercolor;
		}
	}
	...
	public int lerpColor(int c1, int c2, float l) {
		colorMode(RGB, 255);
		float r1 = red(c1) + 0.5f;
		float g1 = green(c1) + 0.5f;
		float b1 = blue(c1) + 0.5f;

		float r2 = red(c2) + 0.5f;
		float g2 = green(c2) + 0.5f;
		float b2 = blue(c2) + 0.5f;
