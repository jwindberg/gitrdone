...
public class SoundSpectrm extends PApplet {

	private static final long serialVersionUID = 1L;
	/**
	 * An FFT object is used to convert an audio signal into its frequency
	 * domain representation. This representation lets you see how much of each
	 * frequency is contained in an audio signal. Sometimes you might not want
	 * to work with the entire spectrum, so it's possible to have the FFT object
	 * calculate average frequency bands by simply averaging the values of
	 * adjacent frequency bands in the full spectrum. There are two different
	 * ways these can be calculated: <b>Linearly</b>, by grouping equal numbers
	 * of adjacent frequency bands, or <b>Logarithmically</b>, by grouping
	 * frequency bands by <i>octave</i>, which is more akin to how humans hear
	 * sound. <br/>
	 * This sketch illustrates the difference between viewing the full spectrum,
	 * linearly spaced averaged bands, and logarithmically spaced averaged
	 * bands.
	 * <p>
	 * From top to bottom:
	 * <ul>
	 ...
	 * <li>The spectrum grouped logarithmically into 10 octaves, each split into
	 * 3 bands.</li>
	 * </ul>
	 * 
	 * Moving the mouse across the sketch will highlight a band in each spectrum
	 * and display what the center frequency of that band is. The averaged bands
	 * are drawn so that they line up with full spectrum bands they are averages
	 * of. In this way, you can clearly see how logarithmic averages differ from
	 * linear averages.
	 * <p>
...
//	PFont font;
	...
	public void draw() {
		...
		{
			// since linear averages group equal numbers of adjacent frequency
			// bands
			// we can simply precalculate how many pixel wide each average's
			// rectangle should be.
			int w = width / fftLin.avgSize();
