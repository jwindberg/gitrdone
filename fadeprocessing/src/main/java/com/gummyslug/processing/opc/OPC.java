...
import com.gummyslug.processing.PColor;
...
public class OPC {
	...
	public OPC(PApplet parent) {
		this(parent, AnimationApplet.HOST_NAME, AnimationApplet.HOST_PORT);
	}

	private OPC(PApplet parent, String host, int port) {
		this.host = host;
		this.port = port;
		this.parent = parent;
...
//			float angle = 0.0f;
	...
	public void led(int index, int x, int y) {
		...
		pixelLocations[index] = x + parent.getWidth() * y;
	}
	...
	public Pixel getPixel(int index) {
		...
		int y = pixelLocations[index] / parent.getWidth();
		int x = pixelLocations[index] - (y * parent.getWidth());
	...
	public void draw() {
		...
		for (int i = 0; i < numPixels; i++) {
			...
			if (enableShowLocations) {
				parent.pixels[pixelLocation] = 0xFFFFFF ^ pixel;
			}
		}
	...
	public void setPixel(int number, PColor c) {
