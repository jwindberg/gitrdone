...
public class OpenPixelControl {
	...
	OpenPixelControl(String host, int port, int height, int width) {
		this.host = host;
		this.port = port;
		this.height = height;
		this.width = width;
		setPixelCount(height * width);
	}
	...
	void led(int index, int x, int y) {
		...
		pixelLocations[index] = x * y;
	}

	// Set the location of several LEDs arranged in a strip.
	// Angle is in radians, measured clockwise from +X.
	// (x,y) is the center of the strip.
	void ledStrip(int index, int count, double x, double y, double spacing, double angle, boolean reversed) {
	...
	void ledGrid(int index, int stripLength, int numStrips, double x, double y, double ledSpacing, double stripSpacing,
			double angle, boolean zigzag) {
		double s = Math.sin(angle + Math.PI / 2);
		double c = Math.cos(angle + Math.PI / 2);
		for (int i = 0; i < numStrips; i++) {
			ledStrip(index + stripLength * i, stripLength, x + (i - (numStrips - 1) / 2.0) * stripSpacing * c, y
					+ (i - (numStrips - 1) / 2.0) * stripSpacing * s, ledSpacing, angle, zigzag && (i % 2) == 1);
		}
	}

	// Set the location of 64 LEDs arranged in a uniform 8x8 grid.
	// (x,y) is the center of the grid.
	void ledGrid8x8(int index, double x, double y, double spacing, double angle, boolean zigzag) {
	...
	void setColorCorrection(double gamma, double red, double green, double blue) {
	...
	public void setVertical(int x, Color color) {
		for (int y = 0; y < getWidth(); y++) {
			setPixel(x, y, color);
		}
	}

	public void setHorizontal(int y, Color color) {
	...
	public void setPixel(int x, int y, Color color) {
		setPixel(getPosition(x, y), color);
	}
	...
	public int getWidth() {
		return (int) Math.sqrt(getNumPixels());
	}
	...
	void setPixel(int number, Color c) {
		...
		packetData[offset] = (byte) c.red;
		packetData[offset + 1] = (byte) c.green;
		packetData[offset + 2] = (byte) c.blue;
	}
	...
	Color getPixel(int number) {
		...
		return new Color(packetData[offset], packetData[offset + 1], packetData[offset + 2]);
	}
